...
`npm run preview` is plain `vite preview`, which already serves the SPA fallback, so it is the way to check a direct hit on a deep route. Deployment needs an SPA fallback rewrite to `index.html` covering `/en/*` too, or direct hits on `/projekty` and `/en/projects` 404. No host config file is committed yet; the host has not been chosen. The `hreflang` alternates are built from `window.location.origin`, so they need no configuration when the domain is picked.
